function solve() {
  ...
  function onClick() {
    ...
    class Restaurant {
      ...
      avgSalary() {
        const sumOfSalaries = this.workers.reduce(
    ...
    for (const text of inputTextarea) {
      ...
      let objRestaurant = restaurants.find(
    ...
    } Average Salary: ${theBestRestaurant.avgSalary()} Best Salary: ${theBestRestaurant.workers[0].salary.toFixed(
      2
    )}`;

    const allSortedWorkers = theBestRestaurant.workers.reduce(
      (result, worker) => {
